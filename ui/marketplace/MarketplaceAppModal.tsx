...
const MarketplaceAppModal = ({
  ...
  return (
    <Modal isOpen={Boolean(data.id)} onClose={onClose} size={isMobile ? 'full' : 'md'} isCentered>
      <ModalOverlay />
      ...
      <ModalContent>
        ...
        >
          ...
          <Text variant="secondary" gridColumn={2} fontSize="sm" fontWeight="normal" lineHeight={1}>
            By{nbsp}
            {author}
          </Text>

          <Box gridColumn={{ base: '1 / 3', sm: 2 }} marginTop={{ base: 6, sm: 0 }}>
            <Flex flexWrap="wrap" gap={6}>
              <Flex width={{ base: '100%', sm: 'auto' }}>
                <MarketplaceAppModalLink id={data.id} url={url} external={external} title={title} />
        ...
        <ModalBody>
          ...
          <Box marginBottom={2}>
            {categories.map((category) => (
              <Tag colorScheme="blue" marginRight={2} marginBottom={2} key={category}>
                {t(category)}
              </Tag>
            ))}
          </Box>
        ...
        >
          {site && (
            <Link
              ...
              overflow="hidden"
            >
              <IconSvg name="link" display="inline" verticalAlign="baseline" boxSize="18px" marginRight={2} />

              <Text color="inherit" whiteSpace="nowrap" overflow="hidden" textOverflow="ellipsis">
                {getHostname(site)}
              </Text>
            </Link>
          )}
          ...
          {socialLinks.length > 0 && (
            <List marginLeft={{ sm: 'auto' }} display="grid" gridAutoFlow="column" columnGap={2}>
              {socialLinks.map(({ icon, url }) => (
                <Link
                  ...
                  h={10}
                >
                  <IconSvg name={icon} w="20px" h="20px" display="block" color="text_secondary" />
                </Link>
              ))}
            </List>
          )}
